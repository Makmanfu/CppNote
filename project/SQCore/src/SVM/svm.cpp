...
static char* readline_test(FILE* input)
{
    int len;
...
int svm_scalemain(int argc, char** argv)
{
    int i, index;
...
static int(*info_)(const char* fmt, ...) = &printf;
...
void predict(FILE* input, FILE* output)
{
    ...
    if (predict_probability)
    {
        if (svm_type == NU_SVR || svm_type == EPSILON_SVR)
            info_("Prob. model for test data: target value = predicted value + z,\nz: Laplace distribution e^(-|z|/sigma)/(2sigma),sigma=%g\n", svm_get_svr_probability(model));
        else
        {
    ...
    max_line_len_ = 1024;
    line_ = (char*)malloc(max_line_len_ * sizeof(char));
    while (readline_test(input) != NULL)
    {
        int i = 0;
    ...
    if (svm_type == NU_SVR || svm_type == EPSILON_SVR)
    {
        info_("Mean squared error = %g (regression)\n", error / total);
        info_("Squared correlation coefficient = %g (regression)\n",
            ((total * sumpt - sump * sumt) * (total * sumpt - sump * sumt)) /
            ((total * sumpp - sump * sump) * (total * sumtt - sumt * sumt))
            );
    }
    else
        info_("Accuracy = %g%% (%d/%d) (classification)\n",
...
int svm_predictmain(int argc, char** argv)
{
    FILE* input, *output;
    int i;
    // parse options
    ...
    {
        if (svm_check_probability_model(model) != 0)
            info_("Model supports probability estimates, but disabled in prediction.\n");
    }
