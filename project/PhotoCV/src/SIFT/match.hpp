...
int matchImg(void)
{
    ...
    double d0, d1;
    int n1, n2, k, i, m = 0;
    //img1 = cvLoadImage("..\\Resource\\lena.jpg", 1);
    //img2 = cvLoadImage("..\\Resource\\lena2.jpg", 1);
    img1 = cvLoadImage("..\\Resource\\A.jpg", 1);
    img2 = cvLoadImage("..\\Resource\\B.jpg", 1);
    img1 = cvLoadImage("..\\Resource\\OpenSIFT\\beaver.png", 1);
    img2 = cvLoadImage("..\\Resource\\OpenSIFT\\beaver_xform.png", 1);
    //img1 = cvLoadImage("..\\Resource\\OpenSURF\\img1.jpg", 1);
    //img2 = cvLoadImage("..\\Resource\\OpenSURF\\img2.jpg", 1);
...
int matchImg(int argc, char** argv)
{
    IplImage* img1, *img2, *stacked;
    struct feature* feat1, *feat2, *feat;
    struct feature** nbrs = NULL;
...
int SiftFeatmain(void)
{
    IplImage* img;
    struct feature* features;
    int n = 0;
    char* img_file_name = "..\\Resource\\OpenSIFT\\beaver.png";
    char* out_file_name = "..\\Resource\\OpenSIFT\\beaver.sift";;
    char* out_img_name = NULL;
    int display = 1;
    int intvls = SIFT_INTVLS;
    double sigma = SIFT_SIGMA;
...
int SiftFeatmain(int argc, char** argv)
{
    IplImage* img;
    struct feature* features;
    int n = 0;
...
/*
    ...
    argc and argv should be passed directly from the command line
*/
static void arg_parse2(int argc, char** argv)
{
    pname = basename(argv[0]);
    int arg = 0;
    while (1)
...
int dspfeatmain(void)
{
    IplImage* img;
    struct feature* feat;
    char* name;
    int n;
    char* feat_file = "..\\Resource\\OpenSIFT\\beaver.sift";
    char* img_file = "..\\Resource\\OpenSIFT\\beaver.png";
    int feat_type = FEATURE_LOWE;
...
int dspfeatmain(int argc, char** argv)
{
    IplImage* img;
    struct feature* feat;
    char* name;
    int n;
    arg_parse2(argc, argv);
    img = cvLoadImage(img_file, 1);
    if (!img)
        fatal_error("unable to load image from %s", img_file);
    n = import_features(feat_file, feat_type, &feat);
