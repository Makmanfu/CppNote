...
namespace stasm
{
    ...
    static bool        gray_g = true;      // gray (monochrome) button
    ...
    static const char* const TOOLTIPS[] =
    {
                        ...
                        if (!OpenWithShell(path1))
                            if (!OpenWithShell(path2))
                                UserMsg(hwnd_child_g,
                                        ...
                                        "(This is a known issue with Wasm\n"
                                        "and is under investigation.\n"
                                        "Close Wasm and try again?)",
                                        path1, path2);
                        EndDialog(hDlg, 0);
                        return true;
                    }
    ...
    static void OpenImg(void)
    {
        ...
        else
        {
            ...
            //
            // The OpenCV function imread crashes on some images.  The try block is a
            // work around.  On our machines we see the crash with the vc10 build of
            // OpenCV 2.4.0 on (our version of) LFPW BlochPortrait1.jpg (which is actually
            // an HTML file in spite of its name).  Photoshop on the same file says
            // "Cannot open filename because a JPEG marker segment is too short".
            //
            // TODO why is this happening?
            try
            {
            ...
            {
                rawimg_g.create(0, 0);    // set rawimg_g.data to NULL
                ClearLastErr();        // ugly
            }
            if (!rawimg_g.data)
            {
                char s[SLEN];
                sprintf_s(s, SLEN, "Cannot read\n%s", img_path_g);
    ...
    static void PossiblyInitCacheImg(void)
    {
        if (done_initfind_g)
        {
            ...
            if (path[0] && strcmp(path, cache_path_g))      // changed?
            {
                ...
                // The OpenCV function imread crashes on some images.  Hence the try block.
                try
                {
                ...
                {
                    cache_rawimg_g.create(0, 0);    // set cache_rawimg_g.data to NULL
                    ClearLastErr();              // ugly
                }
                if (cache_rawimg_g.data)
                {
    ...
    void Wm_Command(HWND hwnd, UINT, WPARAM wParam, LPARAM)
    {
        ...
        {
            case IDM_Open:
                CloseUserMsg(); // remove possible popup message window
                IdmOpen();
                break;
            case IDM_PrevImg:
                CloseUserMsg();
        ...
        LPSTR     lpCmdLine)
    {
        ...
        StateFromRegistry(x, y, width, height);
    ...
    int)
{
    ...
    catch (...)
    {
        ...
        char s[stasm::SBIG];
        if (stasm::img_path_g[0])
            sprintf_s(s, stasm::SBIG, "%s\nCurrent image: %s",
                      stasm_lasterr(), stasm::img_path_g);
        else
            sprintf_s(s, stasm::SBIG, "%s", stasm_lasterr());
        MessageBox(NULL, s, "Wasm Error", MB_OK);
        return 1;   // failure
    }
    return 0;       // success
}
