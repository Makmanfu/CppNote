...
#define _ 0
    ...
    static const FitParams FITPARAMS[] = // 77 landmarks
    {
        { _, _,   }, // 00 L_LTemple
        { _, _,   }, // 01 L_LJaw01
...
#undef _
    ...
    void Fm29(                 // this can be used only on 77 point shapes, else err
    ...
    {
        if (shape.rows != 77)
            Err("Fitness measure FM29 cannot be used on a %d point shape", shape.rows);
