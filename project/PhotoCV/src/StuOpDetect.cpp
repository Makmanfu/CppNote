...
void StuSVM::DemoShow(void)
{
    ...
    //    "D:/Work/CPP/PRO/CppNote/PhotoCV/exe/POS/",
    //    "D:/Work/CPP/PRO/CppNote/PhotoCV/exe/NEG/");
    //定位分类
...
int StuDetect::SpeedSignCheck(Mat& fsrc, Check_Info& CFGinfo) const
{
    ...
    for (int x = 0; x < hsv_base.rows; x++)
    {
        for (int y = 0; y < hsv_base.cols; y++)
        {
            ...
            if (h_src >= 300 && h_src < 360)
                ((float*) how_red.data) [x * hsv_base.cols + y] = static_cast<float>(s_src * sin((h_src - 300.0)
                        / 180.0 * pi) / sin(60.0 / 180 * pi));
            else if (h_src >= 0 && h_src <= 60)
                ((float*) how_red.data) [x * hsv_base.cols + y] = static_cast<float>(s_src * sin((60.0 - h_src)
                        / 180.0 * pi) / sin(60.0 / 180 * pi));
            else
    ...
    for (vector<vector<cv::Point> >::const_iterator itContours = contours_tmp.begin(); itContours != contours_tmp.end(); ++itContours)
    {
        ...
        //计算纵横比
        float rate = static_cast<float>(aRect.height) / aRect.width;
        //计算圆
        int Ri = (aRect.height + aRect.width) / 4;
        float rater = static_cast<float>((pi * Ri * Ri) / tmparea);
...
int StuDetect::SpeedSignVec(Mat& f_src, vector<Rect>& f_faces) const
{
    ...
    cv::erode(RefinedImg, RefinedImg, cv::Mat());    //5ms
    //提取区域寻找红孔
    vector<vector<cv::Point>> contours;
    cv::findContours(RefinedImg, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);    // retrieve all pixels of each contours
    //筛选区域，太大的太小的去除
    ...
    //填充区域。
    cv::drawContours(resultt, contours2, -1, Scalar(255), CV_FILLED);
    cv::minMaxLoc(resultt, min_value, max_value);       //求凸多边形耗时35ms
    //显示红孔
    cv::Mat Hole(img.rows, img.cols, CV_8UC1);
    for (int x = 0; x < img.rows; x++)
        for (int y = 0; y < img.cols; y++)
...
{
    f_faces.clear();
    Ptr<Check_Info> Sign_Cfg(new Check_Info);    //    Check_Info* Sign_Cfg = new Check_Info;   //delete Sign_Cfg;
...
void StuOpDetect::corecv_rotateImage(Mat& f_img, double angle) const
{
    ...
    //createAlphaMat(f_img);
    //imshow( "test", f_img );
    //waitKey(0);
}

void StuOpDetect::createAlphaMat(Mat& f_img)
{
    //注意读PNG图片用 -1
    for (int i = 0; i < f_img.rows; ++i)
        for (int j = 0; j < f_img.cols; ++j)
        {
            Vec4b& rgba = f_img.at<Vec4b> (i, j);
            if (rgba[0] < 10 && rgba[1] < 10 && rgba[2] < 10)
                rgba[3] = 0;    //透明
        }
}
