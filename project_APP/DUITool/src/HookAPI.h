...
public:
    ...
    static void EnableCreateFile(bool bEnable = true)
    {
        m_bCreateFileEnabled = bEnable;
    }
    static void SetSkinDir(LPCTSTR pstrDir)
    {
        _tcscpy_s(m_sSkinDir, pstrDir);
    }

    static void WINAPI Hook_Invalidate(RECT& rcItem);
    static void EnableInvalidate(bool bEnable = true)
    {
        m_bInvalidateEnabled = bEnable;
    }

    static TImageInfo* WINAPI Hook_GetImageEx(LPCTSTR pstrBitmap, LPCTSTR pstrType, DWORD mask);
    static void EnableGetImageEx(bool bEnable = true)
    {
        m_bGetImageExEnabled = bEnable;
    }
