...
#pragma warning(disable: 4996)
    ...
    template <> struct Conversion<wide>
    {
        ...
        typedef LPSTR   ToT;
        enum { TO = ansi };
        static __inline int Convert(UINT cp, CFromT strW, int lenW, ToT strA, int lenA)
        {
            return WideCharToMultiByte(cp, 0, strW, lenW, strA, lenA, NULL, NULL);
        }
        static __inline FromT  Alloc(size_t len)
        {
            return StringAllocW(len);
        }
        static __inline size_t StrLen(CFromT strW)
        {
            return wcslen(strW);
        }
        static __inline size_t BufferSize(size_t len)
        {
    ...
    template <> struct Conversion<ansi>
    {
        ...
        typedef LPWSTR ToT;
        enum { TO = wide };
        static __inline int Convert(UINT cp, CFromT strA, int lenA, ToT strW, int lenW)
        {
            return MultiByteToWideChar(cp, 0, strA, lenA, strW, lenW);
        }
        static __inline FromT  Alloc(size_t len)
        {
            return StringAllocA(len);
        }
        static __inline size_t StrLen(CFromT strA)
        {
            return strlen(strA);
        }
        static __inline size_t BufferSize(size_t len)
    ...
    template <FROM kind> struct Duplicate
    {
        ...
        __inline operator FromT() const
        {
            return out;
        }
    };

    LPSTR  WideToAnsi(LPCWSTR str, int len)
    {
        return Decode<wide,    CP_ACP >(str, len);
    }
    LPSTR  WideToUtf8(LPCWSTR str, int len)
    {
        return Decode<wide,    CP_UTF8>(str, len);
    }
    LPWSTR AnsiToWide(LPCSTR  str, int len)
    {
        return Decode<ansi,    CP_ACP >(str, len);
    }
    LPWSTR Utf8ToWide(LPCSTR  str, int len)
    {
        return Decode<ansi,    CP_UTF8>(str, len);
    }
    LPSTR  AnsiToUtf8(LPCSTR  str, int len)
    {
        return Recode<CP_ACP,  CP_UTF8>(str, len);
    }
    LPSTR  Utf8ToAnsi(LPCSTR  str, int len)
    {
        return Recode<CP_UTF8, CP_ACP >(str, len);
    }
    LPWSTR WideStrDup(LPCWSTR str, int len)
    {
        return Duplicate<wide>(str, len);
    }
    LPSTR  AnsiStrDup(LPCSTR  str, int len)
    {
        return Duplicate<ansi>(str, len);
    }

    //memory routines
    LPWSTR StringAllocW(size_t len)
    {
        return (LPWSTR)__malloc(len * sizeof(wchar_t));
    }
    LPSTR  StringAllocA(size_t len)
    {
        return (LPSTR)__malloc(len);
    }
    void   StringFreeW(LPWSTR strW)
    {
        __free(strW);
    }
    void   StringFreeA(LPSTR strA)
    {
        __free(strA);
    }
